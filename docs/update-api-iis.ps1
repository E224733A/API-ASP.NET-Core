...
Etat final DNS :
- URL finale : http://api.mobilesli.intra:5000
- IIS doit garder le binding http *:5000:api.mobilesli.intra
- les anciens accès par IP ou SRVAPI1.SLI.local peuvent retourner 400 après nettoyage strict DNS-only.
#>
...
function Invoke-RobocopyChecked {
    ...
    if ($LASTEXITCODE -gt 7) {
        throw "$Description échoué. Code robocopy : $LASTEXITCODE"
    }
}
...
function Import-IisModuleOrStop {
    ...
    try {
        ...
        throw "WebAdministration indisponible : $($_.Exception.Message)"
    }
}
...
function Stop-IisApplication {
    ...
    if ((Get-WebAppPoolState -Name $AppPoolName -ErrorAction Stop).Value -eq "Started") {
...
function Start-IisApplication {
    ...
    if ((Get-Website -Name $SiteName -ErrorAction Stop).State -ne "Started") {
...
function Deploy-PublishedFiles {
    ...
    Invoke-RobocopyChecked -Source $PublishPath -Destination $DeployPath -Description "Déploiement"
}
...
function Ensure-WebConfigEnvironment {
    ...
    if ([string]::IsNullOrWhiteSpace($AspNetCoreEnvironment)) {
        Write-Host "ASPNETCORE_ENVIRONMENT non modifié." -ForegroundColor Yellow
        return
    }
...
function Ensure-FinalIisBinding {
    Write-Step "Vérification binding IIS DNS final"

    $expected = "*:${ApiPort}:${ApiDns}"
    $exists = Get-WebBinding -Name $SiteName -ErrorAction SilentlyContinue |
        Where-Object { $_.protocol -eq "http" -and $_.bindingInformation -eq $expected }

    if (-not $exists) {
        New-WebBinding -Name $SiteName -Protocol "http" -Port $ApiPort -IPAddress "*" -HostHeader $ApiDns
        Write-Host "Binding ajouté : $expected" -ForegroundColor Green
    }
    else {
        Write-Host "Binding déjà présent : $expected" -ForegroundColor Green
    }
}
...
function Test-HttpEndpoint {
    ...
    curl.exe -i $Url
    if ($Required -and $LASTEXITCODE -ne 0) {
...
Write-Host "DNS API final  : $ApiDns"
