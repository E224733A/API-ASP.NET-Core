...
function Update-MobilePayloadForTestRun(
    ...
    if ($TestCase.PatchIdSynchronisation) { Set-JsonProperty $Payload 'idSynchronisation' $IdSynchronisation }
    if ($TestCase.PatchCodeTournee) {
    ...
    if ($null -ne $Payload.trajet) {
        if (-not [string]::IsNullOrWhiteSpace((Get-JsonPropertyText $Payload.trajet 'dateDepartMobile'))) {
            Set-JsonProperty $Payload.trajet 'dateDepartMobile' ($DateTourneeText + 'T07:30:00+02:00')
        }
        if (-not [string]::IsNullOrWhiteSpace((Get-JsonPropertyText $Payload.trajet 'dateArriveeMobile'))) {
            Set-JsonProperty $Payload.trajet 'dateArriveeMobile' ($DateTourneeText + 'T16:45:00+02:00')
        }
    }
    ...
    for ($indexLigne = 0; $indexLigne -lt $lignes.Count; $indexLigne++) {
        ...
        if ($null -ne $ligne.saisie) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($heureValidation)) {
                Set-JsonProperty $ligne.saisie 'heureValidation' ($DateTourneeText + 'T09:12:00+02:00')
            }
        }
    }
...
function New-TestCase {
    param(
        ...
        [switch]$RequiresCamionsArray,
        [bool]$PatchIdSynchronisation = $true,
        [bool]$PatchCodeTournee = $true
    )
    ...
    return [pscustomobject]@{
        ...
        PatchIdSynchronisation = [bool]$PatchIdSynchronisation
        PatchCodeTournee = [bool]$PatchCodeTournee
    }
}
...
$testCases = @(
    (New-TestCase -Id 'MOB-API-001' -Order 1 -Scenario 'Synchronisation valide 1.3 avec trajet' -FileName 'sync-valide.json' -ExpectedHttp 200 -ExpectedStatut 'SUCCESS' -Mode 'BaseSuccess' -CodeSuffix '01'),
    ...
    (New-TestCase -Id 'MOB-API-016' -Order 16 -Scenario 'ROLLS_VIDES livre accepte' -FileName 'sync-valide-rolls-vides-livree.json' -ExpectedHttp 200 -ExpectedStatut 'SUCCESS' -CodeSuffix '16'),
    ...
    (New-TestCase -Id 'MOB-API-022' -Order 22 -Scenario 'Synchronisation 1.3 sans camion' -FileName 'sync-v13-sans-camion.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '22'),
    (New-TestCase -Id 'MOB-API-023' -Order 23 -Scenario 'Synchronisation 1.3 sans idCamion' -FileName 'sync-v13-sans-id-camion.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '23'),
    (New-TestCase -Id 'MOB-API-024' -Order 24 -Scenario 'Synchronisation 1.3 sans kilometrageDepart' -FileName 'sync-v13-sans-km-depart.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '24'),
    (New-TestCase -Id 'MOB-API-025' -Order 25 -Scenario 'Synchronisation 1.3 sans kilometrageArrivee' -FileName 'sync-v13-sans-km-arrivee.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '25'),
    (New-TestCase -Id 'MOB-API-026' -Order 26 -Scenario 'Synchronisation 1.3 kilometrageDepart negatif' -FileName 'sync-v13-km-depart-negatif.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '26'),
    (New-TestCase -Id 'MOB-API-027' -Order 27 -Scenario 'Synchronisation 1.3 kilometrageArrivee negatif' -FileName 'sync-v13-km-arrivee-negatif.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '27'),
    (New-TestCase -Id 'MOB-API-028' -Order 28 -Scenario 'Synchronisation 1.3 kilometrageArrivee inferieur au depart' -FileName 'sync-v13-km-arrivee-inferieur-depart.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '28'),
    (New-TestCase -Id 'MOB-API-029' -Order 29 -Scenario 'Synchronisation 1.3 sans dateDepartMobile' -FileName 'sync-v13-sans-date-depart.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '29'),
    (New-TestCase -Id 'MOB-API-030' -Order 30 -Scenario 'Synchronisation 1.3 sans dateArriveeMobile' -FileName 'sync-v13-sans-date-arrivee.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '30'),
    (New-TestCase -Id 'MOB-API-031' -Order 31 -Scenario 'Synchronisation schemaVersion 1.2 refusee' -FileName 'sync-schema-version-12-refusee.json' -ExpectedHttp 400 -ExpectedStatut 'VALIDATION_ERROR' -CodeSuffix '31')
)

Write-Host ''
Write-Host '=== Tests API Mobile SLI - contrat strict 1.3 ==='
...
foreach ($testCase in ($testCases | Sort-Object Order)) {
    ...
    $actualSchemaVersion = Get-JsonPropertyText $responseJson 'schemaVersion'
    ...
    if (-not [string]::IsNullOrWhiteSpace($testCase.ExpectedSchemaVersion)) {
        $schemaVersionOk = [string]::Equals($actualSchemaVersion, $testCase.ExpectedSchemaVersion, [System.StringComparison]::OrdinalIgnoreCase)
    }
    if ($testCase.RequiresCamionsArray) {
        $camionsArrayOk = Test-JsonArrayProperty $responseJson 'camions'
    }
    ...
    if ($resultStatus -eq 'KO') {
        ...
        if (-not [string]::IsNullOrWhiteSpace($testCase.ExpectedSchemaVersion) -and (-not $schemaVersionOk)) { $message += ' schemaVersion attendu=' + $testCase.ExpectedSchemaVersion + ', obtenu=' + $actualSchemaVersion + '.' }
...
$reportLines = @()
$reportLines += '# Rapport tests API Mobile SLI schemaVersion 1.3 strict'
$reportLines += ''
...
$reportLines += ''
$reportLines += 'Note : les scenarios MOB-API-019 a MOB-API-031 couvrent le futur contrat strict 1.3 camion/trajet. Les echecs ne sont pas masques tant que le code API correspondant n''est pas developpe.'
$reportLines += ''
...
foreach ($result in $results) {
    ...
    $expectedStatut = [string]$result.ExpectedStatut
    $actualStatut = [string]$result.ActualStatut
    if ([string]::IsNullOrWhiteSpace($expectedCode)) { $expectedCode = '-' }
    if ([string]::IsNullOrWhiteSpace($actualCode)) { $actualCode = '-' }
    if ([string]::IsNullOrWhiteSpace($expectedStatut)) { $expectedStatut = '-' }
    if ([string]::IsNullOrWhiteSpace($actualStatut)) { $actualStatut = '-' }
    $reportLines += ('| ' + $result.Id + ' | ' + $scenario + ' | ' + $result.ExpectedHttp + ' | ' + $result.ActualHttp + ' | ' + $expectedStatut + ' | ' + $actualStatut + ' | ' + $expectedCode + ' | ' + $actualCode + ' | ' + $result.Result + ' |')
}
$reportLines += ''
