...
IF DB_ID(N'bd_eric') IS NULL
BEGIN
    CREATE DATABASE [bd_eric];
END;
GO
...
/* ============================================================
   ...
   Version complete corrigee : 1.3.2
   ...
   - Appliquer l'option A : les brouillons Expédition restent dans
     SQLite côté serveur web ; SQL Server conserve uniquement les
     préparations verrouillées et officielles.
   ...
   - Le verrouillage Expédition est global : un seul lot VERROUILLE
     est autorisé par DateTournee + CodeTournee = GLOBAL.
...
CREATE TABLE dbo.Mobile_ExpeditionLotVerrouillage (
    ...
    DateReceptionApi DATETIMEOFFSET(0) NOT NULL
        CONSTRAINT DF_Mobile_ExpeditionLotVerrouillage_DateReceptionApi DEFAULT SYSDATETIMEOFFSET(),
    DateSauvegardeSql DATETIMEOFFSET(0) NOT NULL
        CONSTRAINT DF_Mobile_ExpeditionLotVerrouillage_DateSauvegardeSql DEFAULT SYSDATETIMEOFFSET(),
    DateCreation DATETIMEOFFSET(0) NOT NULL
...
/* ============================================================
   ...
   Rôle :
   En-tête d'une préparation Expédition officiellement verrouillée
   pour une date et une tournée.

   Décision d'architecture :
   - Les brouillons ne sont pas stockés dans SQL Server.
   - Les brouillons restent dans SQLite côté serveur web Expédition.
   - SQL Server contient uniquement les préparations verrouillées,
     consommables ensuite par le GET mobile.
...
CREATE TABLE dbo.Mobile_ExpeditionPreparation (
    ...
    StatutPreparation NVARCHAR(30) NOT NULL
        CONSTRAINT DF_Mobile_ExpeditionPreparation_StatutPreparation DEFAULT N'VERROUILLEE',
    EstVerrouille BIT NOT NULL
        CONSTRAINT DF_Mobile_ExpeditionPreparation_EstVerrouille DEFAULT 1,
    DateVerrouillage DATETIMEOFFSET(0) NOT NULL,
    IdLotVerrouillage UNIQUEIDENTIFIER NOT NULL,
    EmpreintePayload CHAR(64) NOT NULL,
    ...
    CONSTRAINT CK_Mobile_ExpeditionPreparation_Statut
        CHECK (StatutPreparation = N'VERROUILLEE'),
...
/* ============================================================
   ...
   Rôle :
   Historique des verrouillages, relectures idempotentes,
   refus et tentatives de modification après verrouillage.
============================================================ */
